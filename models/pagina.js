...
Pagina.getPaquetes = function(callback){
	db.query('SELECT * FROM pagina_tipo   ORDER BY tipo_precio ASC', function(err, rows){
		if (err)
			callback(err,null);
		else{
			callback(null,rows);
...
Pagina.seguidores = function (pagina_id,callback) {
	...
	db.query("SELECT * FROM pagina_seguidor WHERE seguidor_pagina_id = ?",pagina_id,function (err,rows) {
		if (err)
			callback(err,null);
		else{
			callback(null,rows);
...
Pagina.addSeccion = function (datos, callback) {
	db.query('INSERT INTO pagina_seccion SET ?',datos, function(err, result){
        if (err) {console.log(err);}
		else{
...
Pagina.addCuenta = function (email, pass, pagina_id, callback) {
	db.query('INSERT INTO pagina_cuenta SET cuenta_email = ? , cuenta_password = ? , cuenta_pagina_id = ?',[email,pass,pagina_id], function(err, result){
		if (err)
			callback(err,null);
		else
			callback(null,result.insertId);
...
Pagina.getExpired = function (usuario_id, callback) {
	db.query("SELECT p.pagina_id,p.pagina_nombre,p.pagina_vencimiento,up.usuario_id from usuario_pagina up INNER JOIN pagina p on p.pagina_id = up.pagina_id where up.usuario_id = ? and NOW()>p.pagina_vencimiento",[usuario_id], function(err, rows){
        if (err)
            callback(err,null);
        else
        	callback(null,rows);
...
Pagina.getPublicaciones = function(pagina_id,callback){
	db.query('SELECT publicacion_id,publicacion_pagina_id,publicacion_titulo,publicacion_destacada, publicacion_destacada as destacar FROM pagina_publicacion where publicacion_pagina_id = ?  ORDER BY publicacion_fecha DESC', pagina_id, function(err, rows){
		if (err)
			callback(err,null);
		else{
			callback(null,rows);
...
Pagina.updatePublicacion = function (publicacion_id, datos, callback) {
	...
	db.query("UPDATE pagina_publicacion set ? WHERE publicacion_id = ?",[datos, publicacion_id], function(err, result){
		if (err)
			callback(err,null);
		else
			callback(null,result);
...
Pagina.getMensajes = function(pagina_id,callback){
	db.query('SELECT * FROM pagina_mensaje where mensaje_pagina_id = ?  AND mensaje_status != 0 ORDER BY mensaje_fecha DESC', pagina_id, function(err, rows){
		if (err)
			callback(err,null);
		else{
			callback(null,rows);
...
Pagina.getGaleria = function(id,callback){
	db.query('SELECT * FROM pagina_galeria where galeria_id = ? ',id, function(err, rows){
		if (err)
			callback(err,null);
		else
			callback(null,rows[0]);
...
Pagina.updateGaleria = function (galeria_id, datos, callback) {
	...
	db.query("UPDATE pagina_galeria set ? WHERE galeria_id = ?",[datos, galeria_id], function(err, result){if (err)
			callback(err,null);
		else
			callback(null,result);
    });
...
Pagina.updateImagen = function (datos, id, usuario_id, callback) {
	...
	db.query("UPDATE pagina_galeria_imagen set ? WHERE imagen_id = ?",[datos, id], function(err, result){
		if (err)
			callback(err,null);
		else
			callback(null,result);
...
Pagina.getSeccion = function(condicion,callback){
	db.query('SELECT * FROM pagina_seccion where seccion_id = ? ', condicion, function(err, rows){
		if (err)
			callback(err,null);
		else
			if (rows[0]==undefined) {
...
Pagina.addBanner = function (data,callback) {
	db.query("INSERT into pagina_banner SET ?",data,function (err,data) {
		if(err)callback(err);
		else callback(null,data);
	});
}
