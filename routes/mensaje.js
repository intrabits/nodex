var express = require('express');
var router  = express.Router();
var async   = require('async');
var config  = require('./../config/config.js');
var auth    = require('./../config/auth.js');


var moment = require('moment');
var passport        = auth.passport;
ensureAuthenticated = auth.ensureAuthenticated;

//	Mo
var Pagina  = require('./../models/pagina.js');
var Imagen 	= require('./../models/imagen.js');


router.get('/mensajes/latest',ensureAuthenticated, function (req, res){
    var usuario_id = req.user.usuario_id;    
    
    Pagina.getMensajesLatest(usuario_id,function( err, data){
        if (err) {            
            console.log("ERROR : ",err);   
            res.json('error');         
        } else {            
            res.json(data);
        } 
    });
    
} );

router.get('/correo/:pagina_id',function (req,res) {
    console.log(req.params.pagina_id);
    res.send(req.params.pagina_id);
});


router.get('/mensajes/unread',ensureAuthenticated, function (req, res){
    var usuario_id = req.user.usuario_id;    
    
    Pagina.getMensajesPaginas(usuario_id,function( err, data){
        if (err) {            
            console.log("ERROR : ",err);   
            res.json('error');         
        } else {            
            res.json(data);
        } 
    });
    
} );


router.get('/:pagina_id/mensajes',ensureAuthenticated, function (req, res){
    var usuario_id = req.user.usuario_id;
    var pagina_id  = req.params.pagina_id;


    Pagina.getMensajes(pagina_id, function( err, data){
        if (err) {            
            console.log("ERROR : ",err);   
            res.json('error');         
        } else {            
            res.json(data);
        } 
    });
    
} );

router.get('/:pagina_id/mensajes/stats/',ensureAuthenticated, function (req, res){
    var usuario_id = req.user.usuario_id;
    var pagina_id  = req.params.pagina_id;


    Pagina.getMensajesStats(pagina_id, function( err, data){
        if (err) {            
            console.log("ERROR : ",err);   
            res.json('error');         
        } else {            
            res.json(data);
        } 
    });
    
} );

router.get('/:pagina_id/mensaje/:mensaje_id',ensureAuthenticated, function (req, res){
    var usuario_id = req.user.usuario_id;
    var mensaje_id  = req.params.mensaje_id;


    Pagina.getMensaje(mensaje_id, function( err, data){
        if (err) {            
            console.log("ERROR : ",err);   
            res.json('error');         
        } else {           

            Pagina.mensajeLeido(mensaje_id, function (err) {
                if (err) {console.log(err);}
                else{
                    res.json(data);
                };
            })             
        } 
    });
    
} );

module.exports = router;